(function() {
    ...
    angular
        ...
        .service("spa-demo.types.currentTypeSubjects", CurrentSubjects);
    ...
    function CurrentSubjects($rootScope, $q, $resource, currentOrigin, APP_CONFIG) {
        ...
        var typeResource = $resource(APP_CONFIG.server_url + "/api/types/:id",{},{
            query: { cache:false, isArray:true }
        });
        ...
        $rootScope.$watch(function(){ return service.typeIdx; }, refresh);
        return;
        ...
        function refresh() {
            ...
            refreshTypes().then(
                function() {
                    var type = service.getCurrentType() ? service.getCurrentType() : service.setCurrentType(0);
                    params["order"]="ASC";
        ...
        function refreshThings(params) {
            ...
            result.$promise.then(
                function(things){
                    var type_result=typeResource.query(params);
                    type_result.$promise.then(
                        function(type_things) {
                            console.log("things: ", things);
                            console.log("type_things: ", type_things);
                            service.things = things.filter(function(thing) {
                               return type_things.find(function (ele) {
                                  return ele.id === thing.thing_id;
                               });
                            });
        ...
        function refreshTypes() {
            ...
            result.$promise.then(
                function(types){
                    service.types=types;
                    service.version += 1;
                    if (!service.typeIdx || service.typeIdx > types.length) {
                        service.typeIdx=0;
                    }
    ...
    CurrentSubjects.prototype.setCurrentType = function(index) {
        if (index >= 0 && this.types.length > 0) {
            this.typeIdx = (index < this.types.length) ? index : 0;
        } else if (index < 0 && this.types.length > 0) {
            this.typeIdx = this.types.length - 1;
        } else {
            this.typeIdx=null;
        }
